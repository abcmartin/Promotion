...
Um herauszufinden, ob das Vorhandensein einer CD44-Proteinexpression im Tumorgewebe mit dem klinischen Outcome korreliert, wurden in Abbildung 5‑4 die Ereigniskurven der CD44-positiven und -negativen Patientenkollektive für die loko-regionäre Tumorkontrolle skizziert. Die entsprechenden Überlebenskurven für die sekundären Endpunkte sind in Abbildung 5‑6 dargestellt. Mit Hilfe des Log-Rank-Tests wurde anschließend jeweils überprüft, ob zwischen den CD44+ und CD44- Ereigniskurven ein signifikanter Unterschied auf einem Signifikanzniveau von 5% besteht. Die univariaten Analysen zeigen eine signifikante Assoziation der CD44-Proteinexpression mit der loko-regionären Tumorkontrolle (HR 9,09, p = 0,008; siehe Abbildung 5‑4) aber nicht mit den sekundären Endpunkten Fernmetastasen-freies Überleben (HR 2,29, p = 0,075) oder Gesamtüberleben (HR 1,78, p = 0,089). Die dazugehörigen Kaplan-Meier Ereigniskurven für die sekundären Endpunkte Fernmetastasen-freies Überleben und Gesamtüberleben sind weiter unten in Abbildung 5‑6 dargestellt.
...
In univariaten Analysen wurde das Patientenkollektiv stratifiziert in CD44-positive und -negative, und die Überlebensdaten beider Gruppen mit Hilfe des Kaplan-Meier-Verfahrens untersucht. Die geschätzten Überlebenszeiten beider Gruppen wurden m.H. des Log-Rank-Tests miteinander verglichen. Die Untersuchung ergab ein signifikant besseres klinisches Outcome einen hochsignifikanten Überlebensvorteil für Patienten mit p16INK4A-positiven Karzinomen (p < 0,001) (Abb.16A)

### Eingebettete Reproduktionsausgabe (automatisch erzeugt)

Die folgenden Abbildungen und numerischen Zusammenfassungen wurden automatisch aus den Analyse-Skripten in `analysis/` generiert und in `results/` gespeichert. Diese Reproduktionsartefakte sollen die Nachvollziehbarkeit der berichteten Analysen verbessern und als Quellen für Abbildungsdateien im endgültigen Manuskript dienen.

- Interobserver Agreement (IHC Scoring)
	- Kappa (unweighted): 1.00 (n=78; z=11.4; p<0.001)
	- Weighted kappa (squared): 1.00 (n=78; z=8.83; p<0.001)
	- Intraclass correlation coefficient (ICC): 1.00 for single- and average-rater estimates (see `results/interobserver.txt`)
	- Files: `results/interobserver_results.rds`, `results/interobserver.txt`

- Kaplan–Meier and Cox proportional hazards (multivariable)
	- Kaplan–Meier plot (CD44+ vs CD44-): `results/km_cd44.png` (risk table: `results/km_cd44_risktable.png`)
	- Cox model (multivariable) saved: `results/cox_model.rds`
	- Cox model summary (selected coefficients):
		- cd44_status1: exp(coef)=0.231 (95% CI 0.0675–0.7899), p=0.0195
		- hpv16_status: exp(coef)=0.554 (95% CI 0.2606–1.1758), p=0.1239
	- PH diagnostics (Schoenfeld residuals): `results/cox_zph.png`

Diese reproduzierbaren Outputs wurden mit den in `analysis/README.md` angegebenen Befehlen erzeugt und können bei Bedarf in höherer Auflösung oder als SVG/PNG-Varianten neu gerendert werden.
